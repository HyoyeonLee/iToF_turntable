...
const int PIN_MOTORPOWER_FEEDBACK = A5;
...
//typedef struct ControlVars{String version;
//                                inputString;       // holds incoming data
//                                      stringComplete;  // to check whether the incoming data is complete
//                                            configuring;
//                                                    inInterrupt;
  ...
  const int nsteps_full =2*2038;//Steps per revolution
  ...
  AccelStepper stepper = AccelStepper(motorInfo.intfc,IN1,IN3,IN2,IN4);    // order might be different for the L298n driver 
...
// RCCW, motor turns anticlock-wise direction and RCW motor turns clock-wise direction
   ...
   switch (motor.state)
    {
      //++++++++++++++++++++ HOME : Take (motor.currentStep) back. -->standby
    case HOME:
    if(motor.currentStep>0){
      stepper.move(-motor.currentStep);
    ...
    //++++++++++++++++++++++ RCW : SW(4) on? Rotate 90 deg
    case RCW:
    //val_SW = digitalRead( PIN_SW );
    //delay(100);
    //if (val_SW == HIGH)
    //{
      stepper.move(nsteps_quarter);
      stepper.runToPosition();
      //val_SW = digitalRead( PIN_SW );
      motor.currentStep += nsteps_quarter;
      motor.state = STANDBY;
...
void serialEvent()
{
    ...
    if (!controller.configuring)
    {
      ...
      interrupts();//Enable to handle the disabled interrupting events
      while (!Serial.available());
      //char data = (char)toupper(Serial.read());
      char data = toupper((char)Serial.read());
      ...
      case 'G':
        if (motor.state == STANDBY){
          ...
          motor.currentStep = motor.stepCount+1;
        }
        break;
      case 'C':
        motor.currentStep = 0;
...
void PrintControllerState()
{
    ...
    Serial.println(controller.version);
    Serial.print(";Motor.Type: ");
    Serial.println(motorInfo.type);
    Serial.print(";Motor.StepsPerCycle: ");
    Serial.println(motorInfo.stepsPerCycle);
    // Motor specific values
    Serial.print(";Motor.Powered: ");
    Serial.println(CheckDrivePowered());
